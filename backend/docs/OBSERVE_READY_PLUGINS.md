...
- **Install official plugins (no hardcoded IPs):** Run `php artisan observe:install-plugins` to copy `docs/observe_plugins_example/` into `storage/app/observe_plugins/`. Use `--force` to overwrite. This installs scripts for every service type shown in the UI: **CPU, Disk Space, Disk Inodes, Load, Memory, Swap, Uptime, Current Users, Processes, SSH, DNS, SMTP, NTP Time, SSL Certificate, MySQL, PostgreSQL**, plus **check_health.sh** (Custom Script). **HTTP, Ping, TCP Port** are built-in (no script needed).
- **Important:** All plugin scripts must use the **host from the UI** via `OBSERVE_HOST_ADDRESS` and **all parameters** (mount, thresholds, port, etc.) from `OBSERVE_CHECK_ARGS`. Do not hardcode IPs or alert criteria; the engine passes what the user enters in Monitored Targets and service configuration.
