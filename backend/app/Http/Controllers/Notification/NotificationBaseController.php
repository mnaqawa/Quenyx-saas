...
/**
 * Sprint 24 — shared security envelope for the QynNotify Notification controllers. Workspace resolved
 * from a REQUIRED `workspace` UUID; QynNotify entitlement enforced; `accessAi` for reads/writes,
 * `can_use_ai` for AI surfaces. `{ success, data }` envelope.
 */
abstract class NotificationBaseController extends Controller
{
    ...
    protected function workspace(Request $request): Project
    {
        ...
        abort_unless(
            $this->entitlements->hasEffectiveModuleAccess($project, 'qynnotify'),
            403,
            'QynNotify is not enabled for this workspace.'
        );
