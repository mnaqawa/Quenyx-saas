        ...
        $instructions = [
            'linux' => [
                ...
                'steps' => [
                    ...
                    'curl -fSL -o quenyx-agent "'.$apiGatewayUrl.'/api/agents/download/linux-amd64"',
                    '',
                    '2. Verify the download is a real binary (not a JSON error page):',
                    'file quenyx-agent | grep -qE "ELF|executable" || { echo "ERROR: Agent binary not available on server. Ask your admin to run: php artisan agent:build linux-amd64"; head -3 quenyx-agent; exit 1; }',
                    '',
                    '3. Make it executable and enroll (outbound HTTPS to QAG only):',
                    'chmod +x quenyx-agent',
                    './quenyx-agent enroll --url="'.$agentGatewayUrl.'" --workspace='.$workspaceId.' --token="'.$token.'"',
                    '',
                    '4. Install as a systemd service (creates user quenyx if missing, migrates config, enables + starts):',
                    'sudo ./quenyx-agent install',
                    'sudo systemctl status quenyx-agent --no-pager',
                    '',
                    'After upgrading the agent binary on an existing host:',
                    'sudo systemctl stop quenyx-agent',
                    'sudo cp ./quenyx-agent /opt/quenyx-agent && sudo chmod 755 /opt/quenyx-agent',
                    'sudo systemctl start quenyx-agent',
                    'sudo journalctl -u quenyx-agent -n 20 --no-pager',
                    '',
                    'If the service was installed earlier and fails with status=217/USER, recreate it:',
                    'sudo ./quenyx-agent install',
                ],
            ],
            'windows' => [
                ...
                'steps' => [
                    ...
                    '',
                    '2. Verify the file size is at least 1 MB (small files are usually error messages):',
                    'if ((Get-Item quenyx-agent.exe).Length -lt 1MB) { Get-Content quenyx-agent.exe; throw "Agent binary not available on server" }',
                    '',
                    '3. Run enrollment (PowerShell as Administrator):',
            ...
            'macos' => [
                ...
                'steps' => [
                    '1. Download the Quenyx Platform Agent:',
                    'curl -fSL -o quenyx-agent "'.$apiGatewayUrl.'/api/agents/download/darwin-amd64"',
                    '',
                    '2. Verify the download is a real binary:',
                    'file quenyx-agent | grep -q "Mach-O" || { echo "ERROR: Agent binary not available on server"; head -3 quenyx-agent; exit 1; }',
                    '',
                    '3. Make executable and enroll:',
                    'chmod +x quenyx-agent',
                    './quenyx-agent enroll --url="'.$agentGatewayUrl.'" --workspace='.$workspaceId.' --token="'.$token.'"',
                    '',
                    '4. Install as launchd service (optional):',
                    'sudo ./quenyx-agent install',
                ],
            ],
        ];
