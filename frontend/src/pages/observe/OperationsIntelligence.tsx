...
export default function OperationsIntelligence() {
  ...
  const stateCounts = health?.service_state_counts
  const topRisks = data?.top_operational_risks ?? []
  const capacityRisks = data?.predicted_capacity_risks ?? []
  ...
  const investigations = data?.recent_ai_investigations ?? []
      ...
      {data && !loading ? (
        <>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <StatCard title={t('opsIntel.stat.criticalServices')} value={String(stateCounts?.critical ?? 0)} detail={`${health?.services_total ?? 0} ${t('opsIntel.stat.total')}`} />
            <StatCard title={t('opsIntel.stat.openAlerts')} value={String(data.open_alert_count ?? 0)} />
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            ...
            <Panel title={t('opsIntel.section.capacityRisks')}>
              {capacityRisks.filter((r) => r.status !== 'healthy').length === 0 ? (
